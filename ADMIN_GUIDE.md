...
### Exporting Season Data (Development Only)

```bash
curl -s http://localhost:3001/admin/export-data > wmv-backup-$(date +%Y-%m-%d).json
```
...
- Share schedule with other admins

The export includes:
- ✅ Participant list (names, Strava athlete IDs)
- ✅ All segments (names, distances, grades, locations)
- ✅ All seasons (dates, active status)
- ✅ All weeks (dates, segments, laps, time windows)
- ❌ OAuth tokens (excluded for security)
- ❌ Results/leaderboards (computed on-read)

### Importing Season Data

Restore data from a backup or transfer from another instance:

```bash
curl -X POST http://localhost:3001/admin/import-data \
  -H "Content-Type: application/json" \
  -d @wmv-backup-2025-11-12.json
```

### Importing Season Data (Development Only)

Restore data from a backup or transfer from another instance:

**What happens:**
1. All existing participants, segments, seasons, weeks are deleted
2. New data is inserted from the JSON file
3. Everything happens atomically (all-or-nothing)
4. If any error occurs, nothing is changed

**What's preserved:**
- ✅ OAuth tokens (not cleared, so participants stay connected)
- ✅ Deletion request history (for audit)
- ✅ Results/leaderboards (recalculated on-demand)

**Invalid records are skipped:**
- Records missing required fields are silently ignored
- Only valid records are imported
- You'll see how many of each type were imported in the response
...
### Workflow: Archiving a Completed Season

```bash
# 1. Export the current state before archival
curl -s http://localhost:3001/admin/export-data > wmv-season-2025-fall-export.json

# 2. Verify the backup file (should contain all seasons)
cat wmv-season-2025-fall-export.json | jq '.data.seasons'

# 3. When ready to start a new season, import the old data on demand to review past results
# (The current system doesn't yet support archival/historical views, but the export/import provides the data)
```

### Workflow: Multi-Admin Setup

If multiple admins need to coordinate:

```bash
# Admin A exports the schedule
curl -s http://localhost:3001/admin/export-data > shared-schedule.json

# Admin B imports it into their local dev environment
curl -X POST http://localhost:3001/admin/import-data \
  -H "Content-Type: application/json" \
  -d @shared-schedule.json

# Now both admins have the same participants and week schedule
```
