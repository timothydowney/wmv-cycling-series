...
description: 'Audit or design modern WMV UI using the canonical Weekly, Season, and Schedule design system as the baseline for application surfaces, including touched admin flows.'
...
- when building or reviewing WMV UI surfaces (Weekly, Season, Schedule, or Admin)
...
4. Decide whether the target surface is primarily a ranking card, a hero header, a compact object card, or a reveal panel.
5. If the surface switches between peer app views such as Weekly, Season, or Schedule, check whether it should reuse the fixed bottom-nav pattern instead of introducing pill tabs, segmented controls, or ad hoc local nav.
...
- Prefer `SegmentCard` for compact object title and metadata styling when there is no ranking-row hierarchy.
...
- Preserve the leaderboard heading hierarchy: hero and section headings should usually remain WMV purple, while `var(--wmv-text-dark)` should be reserved for dense values and participant identity.
