# Gemini Agent Instructions - Strava NCC Scrape
...
- Create temporary files outside `.gemini-scratch/` directory (if needed)
...
- ✅ When doing analysis/reviews, save to `.gemini-scratch/[descriptive-name].md` for user reference
...
**DO NOT:**
- ❌ Create temp files AND also create duplicate analysis in `/tmp/` (use `.gemini-scratch/` only)
...
**When NOT to use `.gemini-scratch/`:**
...
Remember: `.gemini-scratch/` files are for user reference only. Delete them before final commits.
