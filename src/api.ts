...
export const api = {
  ...
  async getWebhookStatus(): Promise<any> {
    const response = await fetch(`${API_BASE_URL}/admin/webhooks/status`, { credentials: 'include' });
  ...
  async getWebhookEvents(limit = 50, offset = 0, since = 604800): Promise<any> {
    ...
    const response = await fetch(`${API_BASE_URL}/admin/webhooks/events?${params}`, { credentials: 'include' });
    if (!response.ok) throw new Error('Failed to fetch webhook events');
    return response.json();
  },

  async getWebhookStorageStatus(): Promise<any> {
    const response = await fetch(`${API_BASE_URL}/admin/webhooks/storage-status`, { credentials: 'include' });
  ...
  async enableWebhooks(): Promise<any> {
    ...
    if (!response.ok) throw new Error('Failed to enable webhooks');
    return response.json();
  },
  ...
  async disableWebhooks(): Promise<any> {
    ...
    if (!response.ok) throw new Error('Failed to disable webhooks');
    return response.json();
  },
  ...
  async renewWebhooks(): Promise<any> {
    ...
    if (!response.ok) throw new Error('Failed to renew webhooks');
    return response.json();
  },
  ...
  async clearWebhookEvents(): Promise<any> {
    ...
    if (!response.ok) throw new Error('Failed to clear webhook events');
    return response.json();
  },
  ...
  async getWebhookEventEnrichment(eventId: number): Promise<any> {
    ...
    if (!response.ok) throw new Error('Failed to fetch enriched event');
    return response.json();
  },
