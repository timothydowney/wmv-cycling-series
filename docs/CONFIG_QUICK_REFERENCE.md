...
- `dev:prod-data` is the production-copy local review preset.
...
- Use `npm run db:fetch-prod` then `npm run dev:prod-data` when you want to inspect the webhook admin UI against a refreshed local production DB copy.
...
**Production-copy local review preset (`.env.prod`, generated by `npm run db:fetch-prod`):**
```dotenv
FRONTEND_URL=http://localhost:5173
BACKEND_URL=http://localhost:3001
...
**Run against the generated production-copy preset:**
```bash
...
| `.env.prod` | Generated local production-copy review preset | When updating the local workflow for reviewing production-like data |
