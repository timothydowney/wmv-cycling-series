...
Most local confusion came from treating all env vars like they do the same job. They do not.

| Knob | What it controls | Typical values | Who should care |
|------|------------------|----------------|-----------------|
...
- Use `WMV_RUNTIME_MODE=e2e` only for Playwright/E2E harness behavior.
- Use `STRAVA_API_MODE=fixture` when you want deterministic backend Strava behavior locally.

## Recommended Presets

| Scenario | Command | Env file | Runtime mode | Strava mode | Ports |
|----------|---------|----------|--------------|-------------|-------|
| Normal local development | `npm run dev` | `.env` | `default` | Usually `live` | Frontend `5173`, backend `3001` |
| Review webhook admin UI with production-like data | `npm run db:fetch-prod` then `npm run dev:prod-data` | `.env.prod` | `default` | Usually `live` | Frontend `5173`, backend `3001` |
| Playwright E2E | `npm run test:e2e` | `e2e/.env.e2e` | `e2e` | `fixture` | Frontend `5174`, backend `3002` |

If you remember one thing, remember this:
- `dev` is your everyday app.
- `dev:prod-data` is the production-snapshot local review preset.
- `test:e2e` is the dedicated Playwright preset.

For webhook admin review specifically:
- Use `npm run dev` for normal local development and real local Strava-backed work.
- Use `npm run db:fetch-prod` then `npm run dev:prod-data` when you want to inspect the webhook admin UI against a refreshed local Postgres snapshot of production data.
...
**Production-snapshot local review preset (`.env.prod`, generated by `npm run db:fetch-prod`):**
```dotenv
FRONTEND_URL=http://localhost:5173
BACKEND_URL=http://localhost:3001
DATABASE_URL=postgresql://wmv:wmv@localhost:5432/wmv_prod_snapshot
```
...
**Run against the generated production-snapshot preset:**
```bash
npm run db:fetch-prod
npm run dev:prod-data
```

Use this when you want to inspect the webhook admin UI against a realistic local snapshot of webhook events and related production data.
...
### "Which env vars do I actually need to think about?"

Usually only these:
- `ENV_FILE` picks the preset.
- `STRAVA_API_MODE` changes backend Strava behavior.
- `WMV_RUNTIME_MODE` is for E2E-only runtime wiring.

Everything else is normal app configuration such as URLs, database path, secrets, and admin IDs.
...
| `.env.prod` | Generated local production-snapshot review preset | When updating the local workflow for reviewing production-like data |
