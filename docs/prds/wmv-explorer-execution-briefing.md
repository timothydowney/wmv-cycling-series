...
- The next overall implementation step should be the broader app auth-access tightening slice so logged-out users see only a branded WMV sign-in or join shell by default.
- That auth slice should keep the signed-out copy generic, oriented around joining Western Mass Velo with Strava, and should not leave alternate public destinations such as About reachable before login.
- After that cross-product auth slice lands, follow-on Explorer work should return to planning before approving a later slice such as map or social follow-on work.
