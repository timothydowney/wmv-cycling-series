...
Phase 5E (destination popularity and first-completer recognition) is now approved for implementation. The next slice adds lightweight social context with a Club Activity panel on the Hub showing top/bottom destinations and most-recent first completers, plus first-completer badges on destination cards. Map discovery and social expansion become Phase 5F and 5G work pending later re-approval.
...
| Execution phasing | Phase 5E Approved | The phases doc now records public read exposure (5D-0), public surface cleanup (5D-1), and destination popularity & first-completer recognition (5E) as approved or merged. Map discovery and social expansion move to 5F and 5G. |
...
| Test planning | Phase 5E Ready | Backend tests for first-completer logic and completion counts; frontend tests for Club Activity panel and badge rendering; E2E tests for completion flow integration. |
| Documentation impact plan | Phase 5E Recorded | Planning docs updated to capture Phase 5E scope, Club Activity panel placement, first-completer badge design, and deferred map/social work. |
...
| Evidence | The implemented webhook seam remains valid, 4A through 5D-1 are now merged on `main`, the merged auth-access work locks signed-out users to a single WMV join shell, the public Explorer surface cleanup is now complete, and map or social rollout still requires explicit re-approval. |
| Acceptance criteria | The readiness artifacts clearly state that 5D-0 and 5D-1 have landed, the auth-access outcome is recorded on `main`, and later map or social rollout still needs renewed approval through planning. |
| Next action | Keep the current go decision updated as additional athlete-facing Explorer slices are approved or deferred, and return to planning before naming a map or social implementation PR. |
...
| Evidence | The backend now has an explicit E2E mode in [server/src/config.ts](../../server/src/config.ts), the E2E database seeds from the committed sanitized Postgres snapshot [server/data/wmv_e2e_seed.sql](../../server/data/wmv_e2e_seed.sql), and deterministic read-side Strava behavior now flows through explicit providers in [server/src/services/segmentMetadataProvider.ts](../../server/src/services/segmentMetadataProvider.ts) and [server/src/services/stravaReadProvider.ts](../../server/src/services/stravaReadProvider.ts) rather than through scattered service-level `isE2EMode()` branches. Full validation is green, including `npm test`, `npm run test:e2e`, `npm run typecheck`, `npm run lint`, and `npm run build`. |
