...
- Phase 5E introduces destination popularity metrics and first-completer recognition before map discovery, adding lightweight social context with a "Club Activity" panel on the Hub and first-completer badges on destination cards.
- Map-based discovery is important follow-on work (Phase 5F), but it should start only after the list-first athlete page exists, social context is established, and the map product questions are answered explicitly.
- Social visibility expansion (Phase 5G) can grow later, but a social feed is not the smallest useful first athlete-facing Explorer surface.
...
- Refine athlete Explorer checklist browse UX
